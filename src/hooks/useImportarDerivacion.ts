  ...
  const importarArchivo = async (archivo: File): Promise<DerivacionData[]> => {
    ...
    try {
      ...
      if (!extension || !extensionesValidas.includes(extension)) {
        throw new Error(
          `Tipo de archivo no soportado. Use: ${extensionesValidas.join(', ')}`
        );
      }
      ...
      if (resultadoImportacion.advertencias.length > 0) {
        console.warn('Advertencias de importación:', resultadoImportacion.advertencias);
      }
